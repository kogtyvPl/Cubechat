...
local applicationPath = paths.user.applicationData .. "CubeChat/"
local configPath = applicationPath .. "Options.cash"
local historyPath = applicationPath .. "SaveCash.cash"
...
local channelsText = leftLayout:addChild(GUI.text(1, 1, 0xE1E1E1, "  Общие чаты"))
...
local usersText = leftLayout:addChild(GUI.text(1, 1, 0xE1E1E1, "  Пользователи"))
...
local systemText = leftLayout:addChild(GUI.text(1, 1, 0xE1E1E1, "  Система"))
...
local contactAddButton = contactLayout:addChild(GUI.button(1, 1, contactButtonWidth, contactLayout.height, 0x3C3C3C, 0xA5A5A5, 0x878787, 0xB4B4B4, "Добав."))
local contactRemoveButton = contactLayout:addChild(GUI.button(1, 1, contactButtonWidth, contactLayout.height, 0x4B4B4B, 0xA5A5A5, 0x878787, 0xB4B4B4, "Удалить"))
...
local loginUsernameInput = GUI.input(1, 1, 36, 3, 0xE1E1E1, 0x787878, 0xB4B4B4, 0xE1E1E1, 0x2D2D2D, config.username or "", "Ваш никнейм")
local loginPasswordInput = GUI.input(1, 1, 36, 3, 0xE1E1E1, 0x787878, 0xB4B4B4, 0xE1E1E1, 0x2D2D2D, config.password or "", "Ваш пароль")
local loginPasswordSwitchAndLabel = GUI.switchAndLabel(1, 1, 36, 8, 0x66DB80, 0xE1E1E1, 0xFFFFFF, 0xB4B4B4, "Подключение с паролем:", false)
local loginSubmitButton = GUI.button(1, 1, 36, 3, 0xC3C3C3, 0xFFFFFF, 0x969696, 0xFFFFFF, "Подключится")
            ...
            if lineWithoutDots then
              ...
              if #words >= 2 then
                ...
                elseif words[2] == "JOIN" and username then
                  ...
                  addChatMessage(words[3], username .. " (" .. address .. ") Присоеденился к чату", "!").channelAction = true
                ...
                elseif (words[2] == "QUIT" or words[2] == "PART") and username then
                  ...
                  if words[2] == "PART" and username ~= config.username  then
                    addChatMessage(words[3], username .. " (" .. address .. ") вышел из чата", "!").channelAction = true
                  end
                -- User list
...
loginSubmitButton.onTouch = function()
  ...
  status("Подключение к интернету...")
  ...
  if result then
    ...
    status("Подключение к общему серверу, подождите!...")
    socketWrite("")
  else
    GUI.alert("Ошибка к подключению до сервера: " .. tostring(socketHandle))
  end
end
...
settingsButton.onTouch = function()
  ...
  local slider = backgroundLayout:addChild(GUI.slider(1, 1, 36, 0x66DB80, 0xE1E1E1, 0xFFFFFF, 0xB4B4B4, 0, 500, config.historyLimit, false, "Лимит кеша: ", ""))
  slider.height = 2
...
contactAddButton.onTouch = function()
  ...
  local input = backgroundLayout:addChild(GUI.input(1, 1, 36, 3, 0xE1E1E1, 0x787878, 0xB4B4B4, 0xE1E1E1, 0x2D2D2D, "", "Имя, или: #имя чата"))
  backgroundLayout:addChild(GUI.button(1, 1, 36, 3, 0xB4B4B4, 0xFFFFFF, 0x969696, 0xB4B4B4, "Добавить контакт")).onTouch = function()
    backgroundContainer.hidden = true
